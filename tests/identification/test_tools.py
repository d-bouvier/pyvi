...
import unittest
import numpy as np
from pyvi.identification.tools import (_solver, _cast_sig_complex2real,
                                       _check_parameters,
...
class CastSigComplex2RealTest(unittest.TestCase):
    ...
    def test_imag_mode(self):
        result = _cast_sig_complex2real(self.val, cast_mode='imag')
        self.assertTrue(np.all(result == self.imag))

    def test_real_imag_mode(self):
        result = _cast_sig_complex2real(self.val, cast_mode='real-imag')
        self.assertTrue(np.all(result == self.real_imag))

    def test_cplx_mode(self):
        result = _cast_sig_complex2real(self.val, cast_mode='cplx')
        self.assertTrue(np.all(result == self.val))

    def test_warns(self):
        self.assertWarns(UserWarning, _cast_sig_complex2real, self.val,
                         cast_mode='')
